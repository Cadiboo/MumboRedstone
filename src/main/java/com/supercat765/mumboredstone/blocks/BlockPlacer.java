...
public class BlockPlacer extends BlockContainer{
	...
	private BehaviorDefaultDispenseItem PLACE_BLOCK = new BehaviorPlaceBlock();
	private BehaviorDefaultDispenseItem DROP_ITEM = new BehaviorDefaultDispenseItem();
    ...
    protected void dispense(World worldIn, BlockPos pos)
    {
        ...
        if (tileentitydispenser != null)
        {
            ...
            {// || worldIn.isAirBlock(pos.offset((EnumFacing)blocksourceimpl.getBlockState().getValue(BlockDispenser.FACING)))
                ItemStack itemstack = tileentitydispenser.getStackInSlot(i);
                
                Block b = Block.getBlockFromItem(itemstack.getItem());
                if(itemstack.getItem() instanceof IPlantable && MRConfig.AllowCropPlacement){
                	b = ((IPlantable)itemstack.getItem()).getPlant(worldIn, pos).getBlock();
                }
                if(b instanceof BlockSapling && !MRConfig.AllowSaplingPlacement)b=Blocks.AIR;
                if(!MRConfig.canplacerplace(itemstack.getItem().getRegistryName().toString()))b=Blocks.AIR;
                if(b != Blocks.AIR){
                	tileentitydispenser.setInventorySlotContents(i, PLACE_BLOCK.dispense(blocksourceimpl, itemstack));
                }
                else
                {
                	tileentitydispenser.setInventorySlotContents(i, DROP_ITEM.dispense(blocksourceimpl, itemstack));
                }
            }
        }
    }
